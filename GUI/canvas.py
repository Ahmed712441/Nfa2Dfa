...
class DrawingCanvas(Frame):
    ...
    def __init__(self,root,canvas_width=2000,canvas_height=1300,event_root=None,onselect=None,onrelease=None):
        ...
        self.final_nodes = 0 # number of final nodes
    ...
    def mouse_clicked(self,event):
        ...
        if mouse.get_state() == Mouse_state.circle: # check for mouse state
            ...
            try:
                ...
                n.bind_event(self.node_right_clicked,"<Button-3>")
                self.__count_nodes+=1 # increment number of nodes
            except Exception as e: 
                messagebox.showerror(title=e.title,message=e)
                del n
            
    def node_right_clicked(self,event,canvas_item_id):
        
        if mouse.get_state() == Mouse_state.line:
            node = self.objects[str(canvas_item_id)] 
            if self.connection_node == node:
                messagebox.showerror(title="error",message="Node cannot be epsilon-connected with itself")
            elif not self.connection_node : 
                try:           
                ...
                try:
                    line = self.connection_node.epsilon_connect(node)
                    if line :
                        line.bind_event(self.line_clicked)
                        self.objects[str(line.get_id())] =  line 
                except Exception as e:         
    ...
    def __load_connections(self,nodes_dict,lines):
        ...
        for str_line in lines:
            ...
            if splitted[1] != splitted[2]:
                line = nodes_dict[splitted[1]].connect_node(nodes_dict[splitted[2]],splitted[0])
            else:
                line = nodes_dict[splitted[1]].connect_to_itself(splitted[0])
            line.bind_event(self.line_clicked)
            self.objects[str(line.get_id())] =  line
