...
class TransitionTableWindow(tk.Toplevel):
    
    def __init__(self, master,table,alphabets, **kwargs):
        tk.Toplevel.__init__(self, master, **kwargs)
        self.geometry('700x500+600+200')
        self.attributes('-topmost', True)
        self.title('Transition Table Window')
        self.__table = table
        self.__alphabets = list(alphabets)
        self.__alphabets.sort()
        self.scrollframe = ScrollFrame(self, 10, 0, 0, cspan=3).colcfg(range(3), weight=1).frame
        
        self.fill_row_header()
        self.fill_table()

    def fill_row_header(self):
        
        e = tk.Entry(self.scrollframe, width=30,
                            font=('Arial',16,'bold'))      
        e.grid(row=0, column=0,sticky=(N,W,E,S)) # first empty cell  
        i = 1
        for alphabet in self.__alphabets :
            e = tk.Entry(self.scrollframe, width=30,
                            font=('Arial',16,'bold'))
            e.grid(row=0, column=i,sticky=(N,W,E,S))
            e.insert(tk.END, str(alphabet))
    ...
    def fill_table(self):
        ...
        # i = 1
        nodes = list(self.__table.keys())
        for i in range(len(nodes)) :
            
            e = tk.Entry(self.scrollframe, width=30,
                            font=('Arial',16,'bold'))
            e.grid(row=i+1, column=0,sticky=(N,W,E,S))
            e.insert(tk.END, str(nodes[i]))
            e.config(state=tk.DISABLED,disabledbackground="white",disabledforeground='black')
            
            for j in range(len(self.__alphabets)):

                e = tk.Entry(self.scrollframe, width=30,
                            font=('Arial',16,'bold'))
                e.grid(row=i+1, column=j+1,sticky=(N,W,E,S))
                e.insert(tk.END, str(self.__table[nodes[i]][self.__alphabets[j]]))
                e.config(state=tk.DISABLED,disabledbackground="white",disabledforeground='black')
